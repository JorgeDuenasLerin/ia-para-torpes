...
vgg16_ori = VGG16(weights='imagenet', include_top=False, input_shape=(IMAGE_RES, IMAGE_RES,3))
vgg16_ori.trainable = False # no es necesario pero así haríamos que no se entrene en el fit

vgg16 = Sequential()
firsts_layers = vgg16_ori.layers[:11]

for layer in firsts_layers:
    vgg16.add(layer)
...
def get_dogsCats_activations_fromVGG16(source_dir, num_features, augmented=False):
    # Memory for data
...
train_activations, train_labels = get_dogsCats_activations_fromVGG16(train_data_dir, ntrain, True)
validation_activations, validation_labels = get_dogsCats_activations_fromVGG16(validation_data_dir, nvalid)

save('train_activations_sliced_aug.npy', train_activations)
save('train_labels_sliced_aug.npy', train_labels)
save('validation_activations_sliced_aug.npy', validation_activations)
save('validation_labels_sliced_aug.npy', validation_labels)
